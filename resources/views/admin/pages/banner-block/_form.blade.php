<div class="example">
        ...
        @foreach ($localizations as $locale)
            <div class="tab-pane fade @if($loop->first) show active @endif" id="{{ $locale->name }}" role="tabpanel" aria-labelledby="{{$locale->name}}-tab">
                ...
                <div class="mb-3">
                    ...
                    <textarea class="form-control @error('translations.*.description') is-invalid @enderror" name="translations[{{ $locale->id }}][description]" rows="4"> {{ old('translations.1.description') ?? (isset($bannerBlock) ? $bannerBlock->getTranslatedAttributes($locale->id)->description : '') }} </textarea>
                ...
                @isset($bannerBlock)
                    <input type="hidden" name="translations[{{ $locale->id }}][id]" value="{{ $bannerBlock->getTranslatedAttributes($locale->id)->id }}" />
                @endisset
            </div>
        @endforeach
    </div>
    ...
    @isset($bannerBlock)
    <div class="mt-3 mb-3">
        <img src="{{ asset(banner_block_file_path() . $bannerBlock->image) }}" alt="Banner Image" width="200">
    </div>
    @endisset
...
<div class="d-flex justify-content-between mt-3">
    <button type="submit" class="btn btn-primary me-2"> @if(isset($bannerBlock)) {{ __('Сохранить') }} @else {{ __('Добавить') }} @endif </button>
</div>
