<div class="example">
        ...
        @foreach ($localizations as $locale)
            <div class="tab-pane fade @if($loop->first) show active @endif" id="{{ $locale->name }}" role="tabpanel" aria-labelledby="{{$locale->name}}-tab">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" name="translations[{{ $locale->id }}][link_title]" class="form-control @error('translations.*.link_title') is-invalid @enderror" value="{{ old('translations.1.link_title') ?? (isset($marketology) ? $marketology->getTranslatedAttributes($locale->id)->link_title : '') }}" placeholder="Введите название" required>
                ...
                @isset($marketology)
                    <input type="hidden" name="translations[{{ $locale->id }}][id]" value="{{ $marketology->getTranslatedAttributes($locale->id)->id }}" />
                @endisset
            </div>
        @endforeach
    </div>
    ...
    @isset($marketology)
    <div class="mt-3 mb-3">
        <img src="{{ asset(marketology_file_path() . $marketology->image) }}" alt="For Marketology Image" width="200">
    </div>
    @endisset
...
<div class="d-flex justify-content-between mt-3">
    <button type="submit" class="btn btn-primary me-2"> @if(isset($marketology)) {{ __('Сохранить') }} @else {{ __('Добавить') }} @endif </button>
</div>
